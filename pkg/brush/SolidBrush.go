...
func UseSolidBrush(canvas *image.NRGBA, brush *SolidBrush, u g.Point2D) {
	var rB, gB, bB, aB float64
	var rA, gA, bA, aA float64

	p0 := g.Point2D{X: math.Round(u.X - brush.InnerRadius), Y: math.Round(u.Y - brush.InnerRadius)}
	p1 := g.Point2D{X: math.Round(u.X + brush.InnerRadius), Y: math.Round(u.Y + brush.InnerRadius)}

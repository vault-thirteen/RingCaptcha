...
func (sb *SimpleBrush) createSample() (sample *image.NRGBA) {
	...
	p := g.Point2D{X: sb.OuterRadius, Y: sb.OuterRadius}
	UseSimpleBrush(sample, br, true, p)

	err := os.SaveImageAsPngFile(sample, `D:\Temp\2\sample.png`) //TODO
	if err != nil {
		log.Println(err)
	}
...
func UseSimpleBrushS(canvas *image.NRGBA, brush *SimpleBrush, blend bool, u g.Point2D) {
	var rB, gB, bB, aB float64
	var rA, gA, bA, aA float64
	...
	case true: // Use blending.
		...
		for x := x0; x <= xMax; x++ {
			for y := y0; y <= yMax; y++ {
				bcS = brush.Colour
				rB, gB, bB, aB = colour.GetColorComponentsS(canvas.At(x, y))
				rA, gA, bA, aA = colour.GetColorComponentsS(brush.GetSample().At(x-x0, y-y0))

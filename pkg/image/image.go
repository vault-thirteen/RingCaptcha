...
func BlendColourOverlay(cB, cA color.Color) (cO color.Color) {
	rB, gB, bB, aB := colour.GetColorComponentsPM(cB)
	rA, gA, bA, aA := colour.GetColorComponentsPM(cA)
...
func MakeRGBA64(r, g, b, a float64) color.RGBA64 {
	return color.RGBA64{
		R: uint16(math.Round(r * colour.C1)),
		G: uint16(math.Round(g * colour.C1)),
		B: uint16(math.Round(b * colour.C1)),
		A: uint16(math.Round(a * colour.C1)),
	}
}
...
func FillCanvasWithHGradient(canvas *image.NRGBA, cL, cR br.BrushColour) (err error) {
	xMax := canvas.Rect.Dx() // dX.
	yMax := canvas.Rect.Dy() // dY.

	minDim := math2.MinInt(xMax, yMax)
	if minDim < 3 {
...
func FillCanvasWithVGradient(canvas *image.NRGBA, cT, cB br.BrushColour) (err error) {
	xMax := canvas.Rect.Dx() // dX.
	yMax := canvas.Rect.Dy() // dY.
